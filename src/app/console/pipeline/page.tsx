...
export default function PipelineTabPage() {
  ...
  // SSE streaming when a run is in progress (replaces 2s polling)
  // Falls back to 2s polling if SSE fails
  useEffect(() => {
    if (!autoRefresh || !selectedPatientId) return;

    // Fallback polling if SSE failed
    if (sseFailed) {
    ...
    async function connectSSE() {
      try {
        const res = await fetch(
          `${RAILWAY_URL}/api/console/patients/${selectedPatientId}/pipeline/stream`,
        ...
        while (true) {
          ...
          for (const msg of messages) {
            ...
            if (eventType === "done" || eventType === "timeout") {
              // Pipeline completed — do a final full fetch to sync state
              fetchPipeline();
              setAutoRefresh(false);
              setRunningPipeline(false);
              return;
            ...
            try {
              ...
              if (eventType === "replay") {
                // Replay events are handled by initial load — skip to avoid duplicates
                continue;
              }

              // Skip token_delta events for pipeline state — they're ephemeral
              if (event.type === "token_delta") continue;

              // Append event to current run in state
              setPipeline(prev => {
                if (!prev) return prev;
                const runs = [...(prev.pipelineRuns || [])];
                if (runs.length === 0) {
                  runs.push({ startedAt: event.timestamp, events: [] });
                }
                const latestRun = { ...runs[runs.length - 1] };
                latestRun.events = [...(latestRun.events || []), event];
                runs[runs.length - 1] = latestRun;
                return { ...prev, pipelineRuns: runs, pipelineLog: latestRun.events };
              });
        ...
        console.error("SSE connection failed, falling back to polling:", e);
        setSseFailed(true);
      }
    }
    ...
    return () => abort.abort();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [autoRefresh, selectedPatientId, sseFailed]);
  ...
  const runPipeline = async () => {
    if (!selectedPatientId) return;
    setSseFailed(false); // retry SSE on new runs
    setRunningPipeline(true);
    setAutoRefresh(true);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3 bg-white rounded-xl shadow-sm border border-slate-200 p-4">
        ...
        {(runningPipeline || legacyIsRunning) && (
        ...
        <span className="ml-auto flex items-center gap-3">
          ...
          {(runs.length > 0 || legacyLog.length > 0 || firstCallPrep) && !runningPipeline && (
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      {!!genEvent?.openingScript && (
                        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-3 border border-blue-200">
                          ...
                          <p className="text-xs text-slate-800 leading-relaxed">&ldquo;{genEvent.openingScript as string}&rdquo;</p>
                        </div>
                      )}
                      {!!genEvent?.hookAnchor && (
...
function LiveStatusLine({ event, autoScroll, onToggleAutoScroll }: { event: PipelineEvent; autoScroll: boolean; onToggleAutoScroll: () => void }) {
  ...
  return (
    <div className="bg-gradient-to-r from-violet-50 via-indigo-50 to-violet-50 rounded-xl border border-violet-200 px-5 py-3 shadow-sm">
      <div className="flex items-center gap-3">
        {/* Animated spinner */}
      ...
      {detail && (
        <div className="text-xs text-violet-600/70 mt-1.5 leading-relaxed">{detail}</div>
      )}
    </div>
  );
}
